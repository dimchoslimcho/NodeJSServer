...
const cors = require('./cors'); //in the same folder so only one . needed
...
.put(cors.corsWithOptions, authenticate.verifyUser, (req,res,next)=>{
  if(req.user._id.equals(dish.comments.author._id)){
  Dishes.findById(req.params.dishId)
  .then((dish) => {
    if(dish != null && dish.comments.id(req.params.commentId) != null){
      if(req.body.rating){
  ...
  .catch((err) => next(err));
    ...
    err = new Error('You are not authorized to update this comments!');
    err.status = 403;
    return next(err);
  }
})
